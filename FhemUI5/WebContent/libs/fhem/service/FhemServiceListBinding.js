...
// Provides the FhemService model implementation of a list binding
sap.ui.define([
	'jquery.sap.global', 
	...
	function(jQuery, ChangeReason, ClientListBinding) {
		...
		/**
		 * Creates a new FhemServiceListBinding.
		 *
		 * This constructor should only be called by subclasses or model implementations, not by application or control code.
		 * Such code should use {@link de.kjumybit.fhem.service.FhemService#bindList FhemService#bindList} on the corresponding model instance instead.
		 *
		 * @param {de.kjumybit.fhem.service.FhemService} oModel FhemService Model instance that this binding is created for and that it belongs to
		 ...
		 *
		 * @class
		 * List binding implementation for FhemService.
		 *
		 * @alias de.kjumybit.fhem.service.FhemServiceListBinding
		 * @extends sap.ui.model.ClientListBinding
		 * @protected
		 */
		var FhemServiceListBinding = ClientListBinding.extend("de.kjumybit.fhem.service.FhemServiceListBinding");
		 ...
		 * @protected
		 */
		FhemServiceListBinding.prototype.getContexts = function(iStartIndex, iLength) {
			...
			if (this.bUseExtendedChangeDetection) {
				...
				try {
					...
					jQuery.sap.log.warning("FhemServiceListBinding: Extended change detection has been disabled as FhemService data could not be serialized.");
				}
			}
		...
		FhemServiceListBinding.prototype.getCurrentContexts = function() {
			if (this.bUseExtendedChangeDetection) {
		 ...
		 */
		FhemServiceListBinding.prototype.updateIndices = function() {
			var i;
		 ...
		 * @private
		 */
		FhemServiceListBinding.prototype.update = function(){
		 ...
		 *
		 * @param {boolean} bForceupdate Force update all bindings
		 *
		 */
		FhemServiceListBinding.prototype.checkUpdate = function(bForceupdate){
